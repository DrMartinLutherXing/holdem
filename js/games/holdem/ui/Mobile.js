CT.require("mgc.lib.card");
CT.require("mgc.lib.Deck");
CT.require("games.holdem.constants");

games.holdem.ui.Mobile = CT.Class({
	CLASSNAME: "games.holdem.ui.Mobile",
	...
	_responses: {
		...
		turn: function(u) {
			...
			if (t == this.acc_name) {
				...
				this.timer.set(core.config.ctmgc.timeout);
				this.timer.start();
		...
		deal: function(u) {
			...
			var d = u.message.data,
				card = new mgc.lib.Card(d.suit, d.value);
		...
		var that = this, _updates = {
			...
			"card_1": function() {
				if (that.card_1._card)
					mgc.lib.card.setCardImage(that.card_1, that.card_1._card.val());
			},
			"card_2": function() {
				if (that.card_2._card)
					mgc.lib.card.setCardImage(that.card_2, that.card_2._card.val());
			},
		...
		var that = this,
			buttonCb = function() {
				...
				if (m == "FOLD")
					...
					move = m;
				//that._update();
				mgc.core.actor.emit(that.name, "move", {
						"move": move,
	...
	init: function(view) {
		...
		this.table_btn = CT.dom.div("", "m-holdem_btn dealer");
		this.account_name = CT.dom.div("", "m-holdem_text account_name");
		this.current_bid = CT.dom.div("", "m-holdem_text current_bid");
		this.card_1 = CT.dom.div("", "m-holdem_card");
		this.card_2 = CT.dom.div("", "m-holdem_card");
		this.current_money = CT.dom.div("",
			"m-holdem_text current_money");
		this.next_bid = CT.dom.div("", "m-holdem_next_bid");
		...
		this.timer = CT.dom.timer(core.config.ctmgc.timeout, !view.isPlayerNode && this.fold_button.onclick);
